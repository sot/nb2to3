...
def refactor_notebook_inplace(rt, path):
    ...
    path_bak = path.with_name(path.stem + '-py2' + path.suffix)
    print('copying {} to {}'.format(path, path_bak))
    shutil.copy2(str(path), str(path_bak))

    print('writing updated notebook {}'.format(path))
    write(nb, str(path))
